...
{
  int32_T i;
  static const int16_T iv2[8] = { -1, 361, 362, 363, 1, -361, -362, -363 };

  (*obj)->isInitialized = 0;

  /* System object Constructor function: vision.BlobAnalysis */
  for (i = 0; i < 8; i++) {
    (*obj)->cSFunObject.P0_WALKER_RTP[i] = iv2[i];
  }
...
void BlobAnalysis_outputImpl(visioncodegen_BlobAnalysis *obj, const boolean_T
  varargin_1[230400], int32_T varargout_1_data[], int32_T varargout_1_size[2])
{
  ...
  while (n < 640) {
    ...
    m = 0;
    while (m < 360) {
      padIdx = (uint32_T)(pixListMinc + idx);
      start_pixIdx = pixIdx;
      if (b_obj->W3_PAD_DW[padIdx] == 255) {
        b_obj->W3_PAD_DW[padIdx] = currentLabel;
        b_obj->W0_N_PIXLIST_DW[pixIdx] = (int16_T)(loop - 1);
        b_obj->W1_M_PIXLIST_DW[pixIdx] = (int16_T)(idx - 1);
        pixIdx++;
        ...
        while (padIdx != 0U) {
          ...
          for (i = 0; i < 8; i++) {
            ...
            if (b_obj->W3_PAD_DW[walkerIdx] == 255) {
              ...
              b_obj->W0_N_PIXLIST_DW[pixIdx] = (int16_T)((int16_T)(walkerIdx /
                362U) - 1);
              b_obj->W1_M_PIXLIST_DW[pixIdx] = (int16_T)(walkerIdx % 362U - 1U);
              pixIdx++;
  ...
  for (i = 0; i < idx; i++) {
    ...
    maxc = 0;
    maxr = 0;
    for (j = 0; j < (int32_T)b_obj->W2_NUM_PIX_DW[i]; j++) {
      loop = j + n;
    ...
    varargout_1_data[(uint32_T)idx + i] = minr + 1;
    varargout_1_data[(idx << 1) + i] = (maxc - m) + 1;
    varargout_1_data[3 * idx + i] = (maxr - minr) + 1;
    pixListMinc += (int32_T)b_obj->W2_NUM_PIX_DW[i];
    n += (int32_T)b_obj->W2_NUM_PIX_DW[i];
  }
